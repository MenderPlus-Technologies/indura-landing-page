  ...
  return (
    <section className="flex flex-col w-full items-center plusJakarta pt-12 md:pt-16 lg:pt-24 px-4 sm:px-8 md:px-12 lg:px-20 xl:px-[120px] relative overflow-hidden bg-[url('/Hero.svg')] bg-cover bg-center bg-no-repeat pb-0">
      <div className="flex flex-col items-center justify-center gap-6 md:gap-8 lg:gap-10 relative self-stretch w-full flex-[0_0_auto]">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 md:gap-[29.27px] relative flex-[0_0_auto] px-2 sm:px-4">
          {appStoreButtons.map((button) => (
            <Button
              key={button.id}
              variant="outline"
              className="relative cursor-pointer flex items-center justify-start w-[120px] sm:w-[140px] md:w-[146.36px] h-[42px] sm:h-[46px] md:h-[48.79px] bg-black rounded-[6px] sm:rounded-[7.32px] overflow-hidden border border-[#a6a6a6] p-0 hover:bg-black/90 transition-all duration-200"
            >
              <img
                className="absolute top-2 sm:top-2.5 left-2 sm:left-2.5 w-[22px] h-[29px]"
                alt={button.iconAlt}
                src={button.icon}
                loading="eager"
              />

              <div className="flex flex-col items-start absolute top-1/2 -translate-y-1/2 left-9 sm:left-10 md:left-11">
                <div className="font-normal text-white text-[9px] sm:text-[10px] md:text-[11px] leading-[1.1] tracking-[0]">
                  {button.topText}
                </div>
                <div className="font-medium text-white text-[12px] sm:text-[14px] md:text-[16px] tracking-[-0.57px] leading-[1.4] whitespace-nowrap">
                  {button.bottomText}
                </div>
              </div>
            </Button>
